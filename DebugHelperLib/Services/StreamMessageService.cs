...
namespace Blish_HUD.DebugHelperLib.Services {
    ...
    public class StreamMessageService : IMessageService, IDisposable {
        ...
        public void Register<T>(Action<T> callback) where T : Message {
            registedCallbacks.AddOrUpdate(typeof(T), t => x => callback((T)x), (t, _) => x => callback((T)x));
        }

        public void Unregister<T>() where T : Message {
            registedCallbacks.TryRemove(typeof(T), out _);
        }
        ...
        public T SendAndWait<T>(Message message) where T : Message {
            return SendAndWait<T>(message, TimeSpan.FromMilliseconds(-1))!;
        }
        ...
        public T? SendAndWait<T>(Message message, TimeSpan timeout) where T : Message {
            ...
            var received = fResetEvent.Wait(timeout);
